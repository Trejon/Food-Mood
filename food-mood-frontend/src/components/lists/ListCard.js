...
class ListCard extends Component {
  ...
  render() {
    ...
    const meals = list && this.props.myMeals ? this.props.myMeals.filter(meal => meal.attributes.list.id === parseInt(this.props.listId)) : null
              ...
              <div className="column">
                <div className="ui fluid card">
                  <div className="content">
                    <div className="header card-name"><h3>{list.attributes.name}</h3></div>
                    <h4>Restaurants and Recipes on this list:</h4>
                    <ul>{listMeals}</ul>
                  </div>
                </div>
                <Link className="ui primary button list-btn" list={list} to={`/lists/${list.id}/edit`}><h5>Edit this list</h5></Link>
            </div>
          </div> 
