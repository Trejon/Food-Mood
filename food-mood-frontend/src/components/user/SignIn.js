...
class SignIn extends React.Component {
  ...
  getJWTToken = (params) => {
    const url = "http://localhost:3001/login"
    const res = fetch(url, {method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify(params) }).then(res => this.setState({
  ...
  handleOnSubmit = (event) => {
    ...
    const token = this.getJWTToken(params)
    debugger
    const url = "http://localhost:3001/login"
    fetch(url, {
      method: 'POST', 
